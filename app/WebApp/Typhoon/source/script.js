        ...
        async function prefGet() {
            await mapLF.getItem("pref").then(async function (value) {
                if (value !== null) {
                    ...
                    // キーが存在しない場合
                    // 新しくデータを取得
                    const response = await fetch("https://kottaro123456.github.io/geojsons/pref.geojson");
                    const data = await response.json();
                    japan_data = data;
                    console.log("Map Loading completed: 'pref', Network");
                    await mapLF.setItem("pref", japan_data);
        ...
        function getArcPoints(center, radius, startAngle, endAngle) {
            const points = [];
            const step = 2; // degrees for smoothness

            let start = startAngle;
            let end = endAngle;

            // JMA angles are clockwise from North (0 deg)
            ...
            for (let a = start; a <= end; a += step) {
                points.push(destinationLatLng(center, radius, a));
            }

            // Ensure end point is included
            if ((end - start) % step !== 0) {
                points.push(destinationLatLng(center, radius, end));
            }

            return points;
        }

        function destinationLatLng(center, distance, bearing) {
        ...
        function drawStormWarningArea(stormArea) {
            if (!stormArea) return;

            if (stormArea.arc) {
                stormArea.arc.forEach(arc => {
                    const center = arc[0];
                    const radius = arc[1];
                    const angles = arc[2];
                    ...
                    if (angles[0] === 0 && angles[1] === 360) {
                        ...
                        const points = getArcPoints(center, radius, angles[0], angles[1]);
                        L.polyline(points, {
                            color: '#ff0000',
                            weight: 2,
                            pane: "shindo70"
            ...
            if (stormArea.line) {
                stormArea.line.forEach(line => {
                    // JMA line is [[lat1, lon1], [lat2, lon2]]
                    L.polyline(line, {
                        color: '#ff0000',
                        weight: 2,
                        pane: "shindo70"
                ...
                Test_typhoonList.forEach(element => {
                    ...
                    $.when(
                        ...
                        if (Test_Json[0][1]["galeWarningArea"]) {
                            ...
                            kyofu_marker = L.circle(kyofu_latlng, { pane: "shindo30", radius: kyofu_radius, weight: 2, opacity: 1, fillColor: "#ffff00", fillOpacity: 0.2, color: "#ffff00" }).addTo(allList);
                        ...
                        // 予報の最後に最新の暴風警戒域（外枠全体）を描画
                        if (howmuch > 1) {
                            drawStormWarningArea(Test_Json[0][howmuch-1]["stormWarningArea"]);
                        }
                    }
                    )
                });
            ...
            for (var index = 0; index < settingsOnOffArray.length; index++) {
                ...
                if (onoff == 1) {
                    if (index == 0) { //日時がオンの時
                        ...
                        if (textMatome == '<div class="leaflet_datetext">') { eval('textMatome += i' + index + ';'); }
                        else { eval('textMatome += "<br>"+i' + index + ';'); }
                    } else { //ほかのところは1たす
                        var indexPlus1 = index + 1;
                        if (textMatome == '<div class="leaflet_datetext">') { eval('textMatome += i' + indexPlus1 + ';'); }
